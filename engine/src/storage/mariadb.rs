        ...
        self.with_conn(|conn| {
            ...
            conn.query_drop(
                "CREATE INDEX IF NOT EXISTS fabric_cells_lookup
                 ON fabric_cells(fabric_id, relation_type, valid_from, valid_to, ordinal, cell_id)",
            )?;
            conn.query_drop(
            ...
            conn.query_drop(
                "CREATE INDEX IF NOT EXISTS fabric_current_lookup
                 ON fabric(id, valid_from, valid_to)",
            )?;
            conn.query_drop(
