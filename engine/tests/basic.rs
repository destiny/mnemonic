...
use mnemonic_engine::{Engine, CellType, ContentFormat};
    ...
    fn test_cell_lifecycle() {
        ...
        let updated_raw = engine.update_cell_content(raw_cell.id, "Updated content".as_bytes().to_vec()).unwrap();
        ...
        let latest_valid_to_ms = latest_raw.valid_to.duration_since(std::time::UNIX_EPOCH).unwrap().as_millis();
        let updated_valid_to_ms = updated_raw.valid_to.duration_since(std::time::UNIX_EPOCH).unwrap().as_millis();
